...
> 这是 OculiX 项目的 Codex 工作上下文文件。Codex 在该仓库中工作时必须阅读此文件。
...
| `MCP` | **Model Context Protocol 服务器**(`oculix-mcp-server` fat jar),把 OculiX 暴露为 MCP 工具,供 Codex Desktop / Codex / Cursor / 自研客户端通过 stdio 或 HTTP 调用。每个工具调用写入 Ed25519 签名 + SHA-256 链式 JSONL 审计日志 | 3.0.2 新增;**LLM 集成主入口** |
...
- **协议**: 标准 [MCP](https://modelcontextprotocol.io/),**LLM 无关** —— Codex / GPT / Mistral / Gemini / Llama 都能驱动
- **传输**: stdio (Codex Desktop 默认) / HTTP (Streamable HTTP,默认 `127.0.0.1:7337/mcp`)
- **工具集** (10+ 个):
